$("document").ready(function() {
  ...
  //hook onto the ul...
  var artist_ul = document.getElementById("artist_ul");
...
var artistObj = {};
  ...
  function buildListItems(myObject)
  {
    ...
    for(i in myObject)
      {
        output += '<li>'; 
        output += '<a href="' + artistsObj[i].link + '">';
        output += '<img src="' + artistsObj[i].img + '"/>';
        output += '<h3>' + artistsObj[i].fullName + '</h3>';
        output += '<h4>' + artistsObj[i].years + '</h4>';
        output += '</li>';
      }
    artist_ul.innerHTML = output;
  }

  buildListItems(artistsObj);
